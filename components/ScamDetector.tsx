        ...
        <div className="bg-white/10 dark:bg-zinc-900/50 backdrop-blur-xl p-6 sm:p-8 rounded-3xl border border-white/20 dark:border-zinc-700 shadow-2xl relative overflow-hidden text-zinc-900 dark:text-white">
            ...
            {step === 'input' && (
                <div className="relative z-10 animate-in fade-in duration-500">
                    ...
                    <form onSubmit={handleScan} className="space-y-4">
                        <div>
                            <label className="block text-sm font-bold mb-2 text-zinc-300">Votre opérateur actuel</label>
                            <select 
                                required
                                value={operator}
                                onChange={e => setOperator(e.target.value)}
                                className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-red-500 outline-none appearance-none"
                            >
                        ...
                        <div>
                            <label className="block text-sm font-bold mb-2 text-zinc-300">Combien payez-vous par mois ? (DH)</label>
                            <input 
                                type="number" 
                                required
                                min="0"
                                ...
                                className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-red-500 outline-none placeholder:text-zinc-600"
                            />
                        </div>
                        
                        <button 
                            type="submit"
                            className="w-full mt-6 bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-xl shadow-[0_0_20px_rgba(220,38,38,0.4)] transition-all hover:scale-[1.02] flex items-center justify-center gap-2 uppercase tracking-wide"
                        >
                            Lancer le diagnostic
                            <Search className="w-5 h-5" />
                        </button>
                    </form>
                    <p className="text-center text-xs text-zinc-400 mt-4">Scan anonyme. 100% Gratuit.</p>
                </div>
            )}
            ...
            {step === 'scanning' && (
                <div className="relative z-10 py-10 text-center animate-in slide-in-from-right-4 duration-500">
                    ...
                    <h3 className="text-xl font-bold mb-2 text-white">Analyse en cours...</h3>
                    <p className="text-zinc-400 text-sm mb-8 h-6">
                        {progress < 30 && "Connexion aux serveurs opérateurs..."}
                        {progress >= 30 && progress < 60 && "Comparaison avec les offres cachées B2B..."}
                        {progress >= 60 && progress < 90 && "Analyse des marges de l'opérateur..."}
                        {progress >= 90 && "Génération du rapport d'économie..."}
                    </p>
                    ...
                    <p className="text-right text-xs font-bold font-mono text-red-400">{progress}%</p>
                </div>
            )}
            ...
            {step === 'result' && (
                <div className="relative z-10 animate-in zoom-in-95 duration-700">
                    <div className="bg-red-500/10 border border-red-500/50 rounded-2xl p-6 text-center mb-6">
                        ...
                        <h3 className="text-2xl font-black text-white mb-2">Alerte Rouge !</h3>
                        <p className="text-red-200">
                            Vos {price} DH / mois chez {operator} sont injustifiés. 
                            Vous payez en ce moment environ <b className="text-red-400 text-lg">45% trop cher</b> par rapport aux prix réels du marché crypté.
                        </p>
                    </div>
                    ...
                    <div className="space-y-3 mb-6">
                        <div className="flex items-center gap-3 text-sm text-zinc-300 bg-white/5 p-3 rounded-lg border border-white/5">
                            ...
                            <span>Nous avons trouvé <b>2 offres identiques</b> sur notre base secrète B2B.</span>
                        </div>
                        <div className="flex items-center gap-3 text-sm text-zinc-300 bg-white/5 p-3 rounded-lg border border-white/5">
                            <CheckCircle2 className="w-5 h-5 text-green-400 shrink-0" />
                            <span>Économie estimée : <b>{Math.floor(Number(price) * 0.45 * 12)} DH / an</b></span>
                        </div>
                    </div>

                    <button 
                        onClick={() => setShowUnlocker(true)}
                        className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-black py-4 rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all hover:scale-[1.02] flex items-center justify-center gap-2 uppercase animate-bounce"
                    >
                        Prouvez-le moi
