...
async function getOffer(slug: string) {
    const { data: offers, error } = await supabase
        .from('scraped_offers')
        .select('*')
    // A real implementation would query by slug, e.g., .eq('slug', slug)
    // Here we'll just try to find one that matches the slug somewhat
            ...
            <div className="pt-24 pb-8 max-w-6xl mx-auto px-4">
               ...
               <nav className="flex items-center text-sm text-zinc-500 mb-8" aria-label="Breadcrumb">
                  <ol className="flex items-center space-x-2">
                    ...
                    <li>
                      <Link href="/offers" className="hover:text-blue-600 transition-colors">Offres</Link>
                    </li>
                    <li><ChevronRight className="w-4 h-4 mx-1" /></li>
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 bg-white dark:bg-zinc-900 rounded-3xl p-8 border border-zinc-200 dark:border-zinc-800 shadow-sm w-full">
                        <div className="flex items-start justify-between mb-8">
                            ...
                            <div className="w-16 h-16 md:w-24 md:h-24 bg-zinc-100 dark:bg-zinc-800 rounded-2xl flex items-center justify-center text-2xl font-bold text-zinc-400 shadow-inner">
                                {offer.provider[0]}
                            </div>
                        </div>
                        ...
                        <div className="prose prose-zinc dark:prose-invert max-w-none mb-8">
                            <p className="text-lg text-zinc-600 dark:text-zinc-400 leading-relaxed">
                                Découvrez en détail le forfait <strong>{offer.name}</strong> de <strong>{offer.provider}</strong>. 
                                Idéal pour vos besoins en télécommunication, cette offre est conçue pour offrir le meilleur rapport qualité/prix au Maroc.
                            </p>
                        </div>
                    ...
                    <div className="w-full lg:w-[400px] shrink-0 sticky top-24">
                        <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border-2 border-blue-500 shadow-xl relative overflow-hidden">
                             ...
                             <div className="relative z-10 text-center">
                                <p className="text-sm font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mb-2">Prix Mensuel</p>
                                <div className="text-5xl font-black text-zinc-900 dark:text-white mb-6">
                                    {offer.price} <span className="text-xl text-zinc-500 font-medium">DH</span>
                                </div>
